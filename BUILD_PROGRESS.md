...
| P12 | Seed, document, harden, verify | DONE | `make verify-p12` | PASS | lint/mypy --strict/tsc/eslint/prettier all clean; 11 hardening tests covering all five degraded conditions; `make demo` rebuilt everything from a wiped warehouse in one command (1,623 batches, 80,954 rows, scenario detected, cache pre-warmed, serving); 7 Playwright E2E green including a new no-page-error/no-unhandled-rejection assertion; 298 tests in the full suite. Nine defects found and fixed: the E2E suite was outside every tsconfig project so typed linting never parsed it; requests were not cancellable (the AbortSignal was dropped); unhandled rejections were invisible; a 404 on every page load; the role-switch E2E test was racy and failing; the ninth screen (insight detail) was never captured; statsmodels printed a ConvergenceWarning into the demo output; the price-volume-mix waterfall was anchored on the counterfactual rather than on the two windows it compares; and **the number verifier had a false pass** — a relative tolerance cannot express a fixed rendering precision, so the same faithful two-decimal rounding verified on large values and failed on small ones |

## Post-P12 hardening pass

A review pass over things earlier phases left behind. Every item was found by reading
the code or the running app, not by a failing gate — the gates were already green.

| # | Found | Fixed |
|---|---|---|
| 1 | `intern` was granted `rows: "region = :user_region"` on `unit_volume`, but the intern role carries no bindings. The compiler failed closed on it, so the role was denied **by accident** rather than by decision, and the eval reported "policies that will not compile: 1". | The grant now matches its sibling operational contract (`order_fill_rate`): `rows: all`. A contract that means "this role sees nothing" says `deny: true`. |
| 2 | Nothing caught (1) at authoring time — it only surfaced when someone ran that exact query. | `check_referential_integrity` now rejects any row filter binding a value its role does not supply, so `make validate-contracts` catches the whole class. Two tests, one of which pins the exact message. |
| 3 | `RunInputs.observed_metrics` was never populated, so every action precondition was *unchecked* and the selector withheld everything. The Actions screen was permanently empty for a reason nobody could see. | The scan now supplies the metrics the warehouse can genuinely answer (`fill_rate_pct`, `discount_depth_pct`, `daily_spend_inr`, `gross_margin_pct`, `price_index`, `max_vif`, `estimator_agreement`). `days_cover` and `cross_serve_headroom_pct` are **deliberately absent** — no mart provides them, and defaulting them to a plausible zero would convert "could not check" into "passed". Result: one action proposed, one withheld on a genuinely *failed* precondition. |
| 4 | The Actions screen read only the first published insight, so it looked empty whenever the priority ranker put a Low-tier movement at the top — a rendering accident presented as an engine decision. | It now reads every published insight, labels each card with the KPI and tier it came from, and names the insights that proposed nothing and why. |
| 4b | The one action that *was* proposed had an expected impact of **−₹1.18 cr** — a recommendation to lose money. The arithmetic was right: the estimated `price_index` elasticity of net revenue is **+0.93** (inelastic demand), so a price cut is priced as a loss. The selector had no sign check. | An action must move the KPI back towards its baseline or it is not proposed, and the refusal is *reported*, not just logged: `ActionSelection` carries `chosen` and `withheld`, the bundle exposes `actions_withheld`, and the screen renders a "considered and not proposed" list with each reason. Two tests, one of them pinning the wrong-direction case at the estimator's real sign. |
| 4c | With actions finally being proposed (3), the recommendation sentence quoted three figures — the central impact and both interval ends — that the bundle carried **no facts for**. The number verifier rejected the faithful sentence and every persona fell back to the template. Measured in the eval: numeric fidelity **79.3%** (n = 58), down from 100%. The unit fixture hand-wrote those facts, which is precisely why the gap survived every earlier test. | `action_numbers()` emits a `NumberFact` for each proposed action's central impact and both interval ends, keyed by action id so two proposals cannot collide. Re-measured on a clean rebuild: **numeric fidelity 100.0% (n = 46) across 8 narratives**. Note what that number does *not* prove — this run proposes no actions (see 4b), so no recommendation sentence is rendered and the new facts are never exercised by the eval. They are covered by a test instead, and that is said here rather than left to be inferred from a green metric. One test that asserts the facts exist for what the selector actually returned, rather than for what a fixture hand-wrote. |
| 5 | An Ask answer had no route back to its evidence. | The answer links to the insight it was rendered from. |
| 6 | Data & sources refreshed only on load, so breaking a feed on Admin changed nothing on screen until a manual reload. | Freshness and DQ re-poll every 10s, and the page says it is polling rather than being pushed to — because a viewer should know which. |
| 7 | Admin had no clock control, though `DemoControls` could already advance time. | `POST /api/demo/advance-clock` (forward only, 1–30 days, bounded at both the schema and the control) plus a card on Admin. Backwards is still not offered: it means a wipe and a full reload, which is a minute of blank screen mid-demo. Two integration tests. |
| 8 | An `UNKNOWN` dimension member reached the UI bare, where it reads as a broken join. It is not — conform buckets an unmatched member and raises a DQ flag, which is how a launch appears before its SKUs reach the product master. | Segments and feed cards mark it `unmapped` with an explanation on hover. Four tests on the predicate, including the near-misses (`UNKNOWN_CHANNEL`, `Unknown Brands Ltd`). |
| 8b | `landing.wrote` logged every period in a batch. A backfill batch covers 900 days, so one landing produced a 900-element log line and a run produced 1,600 of them — burying every error a reader would open the log to find. | The line logs the count and the span (`2023-09-01..2026-02-26`); the manifest still records every period, which is where an auditor looks. One test. |
| 9 | Two files were over the 400-line limit: `screens/Simple.tsx` (461) and `evals/sections.py` (433). `screens/Interactive.tsx` reached 398 while fixing (4) and (7). | Split into `screens/Trust.tsx` + `components/Panel.tsx`, `evals/sections_delivery.py`, and `screens/Admin.tsx`. Largest source file is now 334 lines. |
