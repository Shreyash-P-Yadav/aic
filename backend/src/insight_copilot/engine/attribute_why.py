...
"""Tail of the sample held out for the MAPE diagnostic. A model that fits the sample
and cannot predict the last few months is describing history, not a mechanism."""
    ...
    * ``hac`` for a **differenced** target. Differencing induces a moving-average error
    ...
    def attribute(
        ...
        *,
        ...
        diagnostics = _diagnose(primary, response, matrix)
        ...
        explained = _explained_fraction(primary, response)
        return WhyResult(
            estimates=estimates,
            diagnostics=diagnostics,
...
def _diagnose(result: object, response: np.ndarray, matrix: pd.DataFrame) -> Diagnostics:
    ...
    return Diagnostics(
        ...
        holdout_mape=_holdout_mape(response, matrix),
        max_vif=max(vifs.values()) if vifs else float("nan"),
        n_observations=int(response.size),
    )


def _holdout_mape(response: np.ndarray, matrix: pd.DataFrame) -> float:
...
def _explained_fraction(result: object, response: np.ndarray) -> float:
